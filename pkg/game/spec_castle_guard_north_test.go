...
type castleGuardNorthCombatEngine struct {
	starts [][2]string
}

func (e *castleGuardNorthCombatEngine) StartCombat(attacker, defender combat.Combatant) error {
...
func TestSpecCastleGuardNorth_EntryGatesAndBlockAudience(t *testing.T) {
	cases := []struct {
		name  string
		...
		{
			name: "house owner at room plus two",
			cmd:  "north",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := specCastleGuardNorth(w, player, guard, tc.cmd, ""); got != tc.want {
	...
	t.Run("blocked uses C audience split and no attack", func(t *testing.T) {
		...
		if got := specCastleGuardNorth(w, player, guard, "north", ""); !got {
			t.Fatal("blocked north should be handled")
		}
...
func TestSpecCastleGuardNorth_AutonomousSecondGuardTarget(t *testing.T) {
	w, player, lastMsg := newSpecProcTestWorld(t)
	guard := newSpecProcTestMob(t, w, player.GetRoomVNum(), 10)
	...
	if got := specCastleGuardNorth(w, nil, guard, "", ""); !got {
...
func TestSpecCastleGuardNorth_AutonomousSecondGuardTargetsMob(t *testing.T) {
	w, player, lastMsg := newSpecProcTestWorld(t)
	guard := newSpecProcTestMob(t, w, player.GetRoomVNum(), 10)
	other := newSpecProcTestMob(t, w, player.GetRoomVNum(), 10)
	...
	guard.VNum = 19510
	other.VNum = 19510
	guard.Prototype.ShortDesc = "an elven north guard"
	other.Prototype.ShortDesc = "another elven north guard"
	target.Prototype.ShortDesc = "an elven north target mob"
...
func TestSpecCastleGuardNorth_DoesNotTreatUnregisteredMobAsPeer(t *testing.T) {
	w, player, lastMsg := newSpecProcTestWorld(t)
	guard := newSpecProcTestMob(t, w, player.GetRoomVNum(), 10)
	other := newSpecProcTestMob(t, w, player.GetRoomVNum(), 10)
	other.SetFighting(player.GetName())
