...
// echoes for board operations (e.g. remove).
func (bs *BoardSystem) SetWorld(w BoardWorld) {
	bs.world = w
}

// BoardInfo returns the static definition for the given board type.
// The returned BoardInfo is a copy; callers in other packages must not
// modify the internal board table.
func (bs *BoardSystem) BoardInfo(boardType int) BoardInfo {
...
// isAbbrev returns true if arg is a non-empty case-insensitive prefix of name.
...
// the magic value (boardType + BoardMagic) so the session layer can set up
// the editor. Returns -1 on failure.
func (bs *BoardSystem) WriteMessage(boardType int, ch BoardPlayer, arg string) int {
	bs.mu.Lock()
	defer bs.mu.Unlock()
	...
	if ch.GetLevel() < bs.boards[boardType].WriteLvl {
	...
	if slot == -1 {
		...
		slog.Error("SYSERR: Board: failed to find empty slot on write.")
		return -1
	}
...
func (bs *BoardSystem) ShowBoard(boardType int, ch BoardPlayer) bool {
	bs.mu.RLock()
	defer bs.mu.RUnlock()
	...
	if bs.numOfMsgs[boardType] == 0 {
		...
		for i := 0; i < bs.numOfMsgs[boardType]; i++ {
			if bs.msgIndex[boardType][i].Heading != "" {
				...
				slog.Error("SYSERR: The board is fubar'd.")
...
func (bs *BoardSystem) DisplayMsg(boardType int, ch BoardPlayer, arg string) bool {
	bs.mu.RLock()
	defer bs.mu.RUnlock()
	...
	if mi.SlotNum < 0 || mi.SlotNum >= len(bs.msgStorage) {
		...
		slog.Error("SYSERR: Board is screwed up.")
		return true
	}
	if mi.Heading == "" {
...
func (bs *BoardSystem) RemoveMsg(boardType int, ch BoardPlayer, arg string) bool {
	bs.mu.Lock()
	defer bs.mu.Unlock()
	...
	playerTag := fmt.Sprintf("(%s)", ch.GetName())
	if ch.GetLevel() < bs.boards[boardType].RemoveLvl && !strings.Contains(mi.Heading, playerTag) {
	...
	slot := mi.SlotNum
	if slot < 0 || slot >= len(bs.msgStorage) {
		slog.Error("SYSERR: The board is seriously screwed up.")
