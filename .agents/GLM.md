...
- **Do NOT commit `docs/reports/scenario-coverage-*.tsv`.** It's a generated artifact (`make scenario-coverage`), now gitignored. Committing it in every scenario PR serialized all of them on one file (each regenerates it → add/add conflict → a rebase per PR). Add your scenario + code fix; the coverage number is reproducible on demand and by the cron. Report the probed delta in the PR body, don't commit the report.
