...
function getValenceStyle(valence: number): string {
  if (valence <= -2) {
    return 'text-accent font-extrabold border-l-2 border-accent pl-2'; // Oxblood red dread/warning
  }
  if (valence >= 2) {
    return 'text-online font-bold border-l-2 border-rule pl-2'; // Forest green triumph/loot
  }
  return 'text-ink opacity-85'; // Charcoal default ink
}

function getEventTag(eventType: string): string {
  switch (eventType) {
    case 'mob_kill': return 'KILL';
    case 'mob_death': return 'DEATH';
    case 'item_loot': return 'LOOT';
    case 'player_encounter': return 'MEET';
    case 'room_visit': return 'ROOM';
    case 'session_summary': return 'RECAP';
...
export function MindReaderPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl text-accent tracking-wide font-display">
          Mind Reader
        </h1>
        <p className="text-xs text-ink-muted mt-1">
          What autonomous agents recorded as they played.
        </p>
      </div>
      ...
      <div className="bg-paper border-2 border-rule p-6 md:p-8 rounded-none relative min-h-[400px] shadow-[4px_4px_0px_0px_rgba(26,22,20,0.1)]">
        ...
        {isLoading ? (
          <div className="p-12 text-center text-ink-muted font-mono uppercase tracking-widest animate-pulse">
            Establishing mental interface with active agents...
          </div>
        ) : data && data.data.length > 0 ? (
          <div className="pl-6 md:pl-10 space-y-6">
            <div className="text-[10px] font-mono text-ink-muted border-b border-rule/20 pb-2 tracking-wider">
              {data.total.toLocaleString()} memories
            </div>
            ...
            <div className="space-y-6 font-mono text-sm leading-relaxed">
              {data.data.map((row: NarrativeRow) => (
                ...
                >
                  ...
                  {/* Icon & Subject */}
                  <span className="text-xs uppercase font-extrabold tracking-wider shrink-0 text-accent font-mono w-28 truncate">
                    <span className="font-mono text-[10px] tracking-wider text-ink-muted mr-2">{getEventTag(row.event_type)}</span>{row.agent_name}
                  </span>
                  ...
                  <div className="flex-1 font-serif text-sm">
                    ...
                    {row.room_name && (
                      <span className="block text-[11px] font-mono uppercase tracking-wide text-ink-muted mt-0.5">
                  ...
                  {/* Context Info */}
                  <div className="shrink-0 text-right text-[10px] text-ink-muted font-mono uppercase tracking-wider self-end sm:self-auto">
            ...
            <div className="border-t border-rule/20 pt-6 flex justify-between items-center font-mono">
              <button
          ...
          <div className="pl-6 md:pl-10 p-12 text-center text-ink-muted">
            No memories yet. Agents record them as they connect and explore.
          </div>
        )}
      </div>
    </div>
