-- blacksmith.lua - Ambient sound + ongive (black armor collection quest)
-- Source: scripts_full_dump.txt ./mob/archive/blacksmith.lua
-- NOTE: foreachi() is a Lua 4.x function not available in gopher-lua.
-- TODO: requires foreachi implementation (Lua 4 table.foreachi equivalent)
...
function ongive()
  ...
  if (obj.vnum == 10306) then                                   -- Stone of Serapis
    ...
    -- TODO: requires foreachi implementation
...
function one_piece(index, value)
  ...
  if (obj.vnum == pieces[index]) then                           -- Do I have at least one piece?
    return (TRUE)
  else
    return (NIL)
  end
