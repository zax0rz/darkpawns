-- merchant_walk.lua
-- Source: scripts_full_dump.txt lines 3585-4130
-- Original: Escort quest mob (6805) — travels from Mist Keep gates to Xixieqi (room 4860).
-- Features: pathfinding via direction(), bandit ambush at room 7093, escort tracking,
-- anti-rest enforcement, quest completion with 5000 gold reward.
-- Triggers: onpulse_all, oncmd, fight, death, init_quest, tele_mobs, escort, end_quest, attack_time, clear_quest, extraction
-- State variables: merchant_char, merchant_conv_state, merchant_yesno, merchant_ptr, bandit

function onpulse_all()
  -- Source: scripts_full_dump.txt lines 3587-3679
  local dir = 0
  local buf = ""
  ...
  if (merchant_conv_state ~= 0) then
    ...
    if (position == 0) then
      ...
      -- Source: scripts_full_dump.txt lines 3644-3667
      if (dir == 0) then
        buf = "north"
      elseif (dir == 1) then
        buf = "east"
...
function oncmd()
  -- Source: scripts_full_dump.txt lines 3681-3698
  -- Prevent the player from sitting down, resting or sleeping
  local command = ""
...
function fight()
  -- Source: scripts_full_dump.txt lines 3700-3709
...
function init_quest()
  -- Source: scripts_full_dump.txt lines 3717-3722
  -- Initiate a delay before teleporting quest mobiles to start room
  me = inworld("mob", 6805)
  create_event(me, NIL, NIL, NIL, "tele_mobs", 480, LT_MOB)
end

function tele_mobs()
  -- Source: scripts_full_dump.txt lines 3724-3728
  tport(me, 5315)
...
function escort()               -- Is our escort still here?
  -- Source: scripts_full_dump.txt lines 3730-3749
  local found = FALSE

  for i = 1, getn(room.char) do
    if (room.char[i].name == merchant_char.name) then
      merchant_char = room.char[i]
      found = TRUE
      break
    end
  end
...
function end_quest()            -- Have we reached the end of the quest?
  -- Source: scripts_full_dump.txt lines 3751-3768
...
function clear_quest()          -- Reset all of the quest variables
  -- Source: scripts_full_dump.txt lines 3786-3792
  merchant_char = NIL
  bandit = FALSE
...
function extraction()
  -- Source: scripts_full_dump.txt lines 3794-3805
  -- Extract all non-fighting quest mobs and objs
