...
@dataclass
class AIContext:
    """AI context for testing. Matches the interface expected by test cases."""
    player_id: str = ""
    player_name: str = ""
    room_id: str = ""
    room_name: str = ""
    ...
    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "AIContext":
        return cls(
...
class BehaviorManager:
    """Behavior manager stub for testing."""
    def __init__(self):
    ...
    def evaluate_behavior(self, game_state: Dict, condition_checker=None):
        """Evaluate all registered behaviors and return the highest-priority match."""
        if condition_checker is None:
            condition_checker = lambda c, gs: True

        candidates = []
        for name, b in self.behaviors.items():
            if all(condition_checker(c, game_state) for c in b.conditions):
                candidates.append(b)
    ...
    def execute_behavior(self, name: str, game_state: Dict, action_executor=None):
        """Execute all actions of a registered behavior."""
        behavior = self.behaviors.get(name)
        if behavior is None:
            return []

        if action_executor is None:
            action_executor = lambda a, gs: f"Executed {a}"

        return [action_executor(action, game_state) for action in behavior.actions]
        ...
        for test_case in test_cases:
            ...
            if test_case['expected_behavior']:
                assert selected is not None, \
                    f"[{test_case['name']}] Expected {test_case['expected_behavior']}, got None"
                assert selected.name == test_case['expected_behavior'], \
                    f"[{test_case['name']}] Expected {test_case['expected_behavior']}, got {selected.name}"
            else:
    ...
    def mock_memory_system(self):
        ...
        class MockMemory:
            ...
            async def get_context(self, current_situation):
                ...
                relevant = []
                for event in self.memory['events'][-5:]:
