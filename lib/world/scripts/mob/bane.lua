...
--
-- Bug 2 fix: onpulse_pc fires on a timer, not triggered by a player action, so
-- the 'ch' global is nil.  All create_event() calls in pulse/timer context now
-- use NIL instead of ch.  Same fix applied to every subsequent bane_N() callback
-- since they also fire as timer events, not player-triggered events.
--
-- Bug 3 fix: bane_one() success branch checked (keep_conv_state == 2) but state 2
-- is set by valoran's val_one() function.  The scripts cooperate: bane schedules
-- bane_one at t+10s, valoran schedules val_one at t+6s, val_one sets state=2 and
-- schedules val_two.  bane_one() fires at t+10s and finds state==2.  No change
-- needed in bane.lua itself — the original logic is correct; the bug only manifests
-- if valoran.lua is absent or has its own nil-ch error preventing val_one from running.
...
-- and the player.  onpulse_pc fires on a timer, so ch is nil — do not use ch.
  ...
  if (keep_conv_state == 0) then
    ...
    keep_conv_state = 1
    -- Bug 2 fix: was create_event(me, ch, ...) — ch is nil in onpulse_pc context
    create_event(me, NIL, NIL, NIL, "bane_one", 10, LT_MOB)
  elseif (keep_conv_state == 4) then
    keep_conv_state = 5
    -- Bug 2 fix: was create_event(me, ch, ...)
    create_event(me, NIL, NIL, NIL, "bane_two", 6, LT_MOB)
  end
end

function bane_one()
  -- keep_conv_state == 2 is set by valoran's val_one() which fires ~4s before this.
  -- If val_one found the focus owner, state is 2 (success). Otherwise state is still 1.
  if (keep_conv_state == 2) then
    say("Indeed, and it appears "..keep_focus_owner.name.." has the focus necessary "
      .."for the ceremony.")
    -- Bug 2 fix: was create_event(me, ch, ...)
    create_event(me, NIL, NIL, NIL, "bane_two", 16, LT_MOB)
  else
...
function bane_two()
  ...
  -- Bug 2 fix: was create_event(me, ch, ...)
  create_event(me, NIL, NIL, NIL, "bane_three", 6, LT_MOB)
end
...
function bane_three()
  ...
  -- Bug 2 fix: was create_event(me, ch, ...)
  create_event(me, NIL, NIL, NIL, "bane_four", 10, LT_MOB)
end
...
function bane_four()
  ...
  -- Bug 2 fix: was create_event(me, ch, ...)
  create_event(me, NIL, NIL, NIL, "bane_five", 10, LT_MOB)
end
...
function bane_five()
  ...
  -- Bug 2 fix: was create_event(me, ch, ...)
  create_event(me, NIL, NIL, NIL, "bane_six", 10, LT_MOB)
end
