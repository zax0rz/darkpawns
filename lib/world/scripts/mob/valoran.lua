...
-- 'ch' global is nil.  All create_event() calls in pulse/timer context use NIL.
-- (The original already used NIL here; this port preserves that correct behavior.)
--
-- State machine note: val_one() sets keep_conv_state = 2, which is the signal
-- that enables bane_one()'s success branch (Bug 3 in bane.lua).  val_one fires
-- ~6s after state reaches 1; bane_one fires ~10s later, so state 2 is set in time.
...
-- and the player.  onpulse_pc fires on a timer, so ch is nil — use NIL in all
-- create_event() calls here.
  ...
  if (keep_conv_state == 1) then
    -- Bug 2 fix: NIL (not ch) — timer context, ch is nil
...
function val_one()
  ...
  if (keep_focus_owner) then
    ...
    keep_conv_state = 2		-- enables bane_one() success branch (Bug 3 fix in bane.lua)
