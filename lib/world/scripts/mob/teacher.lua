-- teacher.lua
-- Source: scripts_full_dump.txt lines 5320-5562 (approx)
-- Original: Full skill training system with skill_group(), get_group_lvl(), get_group_pts().
-- Teachers are identified by mob vnum and offer specific skill groups.
-- Advanced teachers (vnums 7971-7973) offer different groups up to level 4.
-- Triggers: code(), oncmd()
-- Engine gaps: skill_group(), get_group_lvl(), get_group_pts(), set_skill(), mxp(), exp(), format()

function code()
  -- Source: scripts_full_dump.txt lines 5321-5427
  local teachers =
  ...
  if (argument == "") then                   -- Display skills/spells to teach
    ...
    if (ch.group_lvl > 0) then
      set_skill(ch, group, (ch.group_lvl * 25) + 5)
    else
      set_skill(ch, group, 20)
    end
...
function oncmd()
  -- Source: scripts_full_dump.txt lines 5429-5475
  -- Players can "unlearn" their groups and have group points refunded.
  local command = ""
  local subcmd = ""
  local buf = ""
  local group_name = ""
