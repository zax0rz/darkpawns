...
| **Server restart / crash recovery** | Not reproducible: one server process per scenario. | DP-1310's first sighting (restart while voided put the player in Limbo). | A `<RESTART>` step: stop both servers, keep their data dirs, start again, relogin. |
