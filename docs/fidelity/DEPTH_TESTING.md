...
Each round's handoff note lives in its own file under
`docs/fidelity/depth/handoff/` (one file per round, named by date). A new
round appends a NEW file there instead of editing this one — there is no
shared insertion point, so consecutive rounds cannot conflict on this
document.
