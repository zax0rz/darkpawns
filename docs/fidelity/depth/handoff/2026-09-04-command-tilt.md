# Depth-fidelity handoff — `tilt`

Date: 2026-09-04

Feature branch: `glm/depth-tilt`

## Queue position and scope

This slice starts from merged main at `f870b9da2` after the `tickle` depth
...
in `src/interpreter.c` is `tilt` at line 777. No `tilt` manifest, scenario,
...
through `act`. The authored record at `lib/misc/socials:1516-1524` is:
...
Added `cmd/dp-oracle-diff/scenarios/tilt-depth.txt` with the standard actor,
observer, target, and generic-mob fixture;
`pkg/session/tilt_depth_test.go` to pin the C command gate, social metadata,
and all eight parsed message slots; and `docs/fidelity/depth/tilt.tsv` with
...
The final `tilt-depth` matrix used the C oracle at seeds 1, 2, 3, 5, and 8.
...
- `make fidelity-depth` — 4,582 total, 4,477 proven/delegated, 54 blocked,
...
The handoff and evidence are in commits `29d4ae55b` and `d91324cb6` at the
time of this note. No file under `src/` or `darkpawns-c-oracle/` was edited.

## Starting frontier

The merged `tickle` handoff reported 4,570 total cases: 4,465
proven/delegated, 54 blocked, and 51 excluded. This slice adds twelve
proven/delegated rows, bringing the frontier to 4,582 total, 4,477
proven/delegated, 54 blocked, and 51 excluded. The next fresh social after
this slice is `tug` at `src/interpreter.c:786`.
