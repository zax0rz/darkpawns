# Depth-fidelity handoff — `spit`

Date: 2026-09-03

Branch: `glm/depth-spit` (feature merged); handoff branch:
`handoff/2026-09-03-command-spit`

Feature PR: #1283 (merged green)

Feature commit: `6be529896`
...
The next unclaimed interpreter-table family after `spike` was the `do_action`
social `spit` at `src/interpreter.c:731`. Its depth vehicle and manifest are
complete; the feature PR was self-merged only after all applicable GitHub
checks were green. The next fresh queue boundary is `squeeze` at
`src/interpreter.c:732`; do not repick `spit` or earlier claimed families.

Pre-spit frontier: 3,878 total, 3,774 proven/delegated, 48 blocked, and 56
excluded. The spit manifest adds 11 proven/delegated cases. Post-spit
frontier: 3,889 total, 3,785 proven/delegated, 48 blocked, and 56 excluded;
actionable completion is 3,785/3,833 = 98.7%.
...
proper-position refusal, or target-success branches. The `spit` record at
`lib/misc/socials:824-832` is `spit 0 0`, with no command minimum, no hidden
flag, the default zero victim-position minimum, and eight authored message
slots.

The reachable player-visible branches are:

- no argument: the authored actor and room left-shoulder lines;
- visible player target: authored actor, non-victim observer, and victim lines;
- visible NPC target: the same authored target audience with NPC substitution;
- leading fill word and trailing words: only the first target token is used;
- self target: the authored drool actor and room lines;
...
The clean-main proof was GREEN at seed 1 with `--show-oracle`; the normalized
oracle blocks exposed every annotated branch, including the NPC and sleeping
target cases. Final `spit-depth` runs at seeds 1, 2, 3, 5, and 8 all reported
`result: no normalized divergence`. No Go product behavior changed; this was
a pure depth-coverage slice.

Durable evidence:

- `cmd/dp-oracle-diff/scenarios/spit-depth.txt`;
- `docs/fidelity/depth/spit.tsv`; and
- `pkg/session/spit_depth_test.go`.
...
PR #1283's hosted lint, security, and full test checks completed green;
...
`squeeze` at `src/interpreter.c:732` before touching any implementation. Do
not repick `spit` or any earlier claimed family.
