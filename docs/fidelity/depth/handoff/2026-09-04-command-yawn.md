# Depth-fidelity handoff — `yawn`

Date: 2026-09-04

Feature branch: `glm/depth-yawn`

## Queue position and scope

This slice starts from merged main at `779b7eb83` after the `worship` depth
slice and the scheduled worktree prune. The special-procedure inventory
remains exhausted; the designated Phase 2 red/blocked families, blocked clinic
vehicles, and the later off-command-table phases remain queued. Phase 1
continues through the remaining socials. The next genuinely unmanifested
reachable `do_action` row in `src/interpreter.c` is `yawn` at line 840. No
`yawn` manifest, scenario, or focused registration test exists on this
starting main.
...
`act`. The authored record at `lib/misc/socials:1024-1027` is:
...
Added `cmd/dp-oracle-diff/scenarios/yawn-depth.txt` with the standard
actor/peer fixture and four self-only probes; `pkg/session/yawn_depth_test.go`
to pin the C command gate, social metadata, and all three parsed message slots;
and `docs/fidelity/depth/yawn.tsv` with eight durable D1-D3 rows. This was a
...
The `yawn-depth` matrix used the C oracle at seeds 1, 2, 3, 5, and 8. Seed 1
...
- `make fidelity-depth` — 4,742 total, 4,637 proven/delegated, 54 blocked,
...
The handoff and evidence are in commits `3bbb226b6` and `8b09c7fb8`. No file
under `src/` or `darkpawns-c-oracle/` was edited.

## Starting frontier

The merged `worship` handoff reported 4,734 total cases: 4,629
proven/delegated, 54 blocked, and 51 excluded. This slice adds eight
proven/delegated rows, bringing the frontier to 4,742 total, 4,637
proven/delegated, 54 blocked, and 51 excluded. The next fresh social is
`yodel` at `src/interpreter.c:841`.
