# Depth-fidelity handoff — `wave`

Date: 2026-09-04

Feature branch: `glm/depth-wave`

## Queue position and scope

This slice starts from merged main at `5a0c4bf03` after the `waltz` depth
...
in `src/interpreter.c` is `wave` at line 808. No `wave` manifest, scenario,
...
through `act`. The authored record at `lib/misc/socials:964-972` is:
...
Added `cmd/dp-oracle-diff/scenarios/wave-depth.txt` with the standard actor,
observer, target, and generic-mob fixture;
`pkg/session/wave_depth_test.go` to pin the C command gate, social metadata,
and all eight parsed message slots; and `docs/fidelity/depth/wave.tsv` with
...
The final `wave-depth` matrix used the C oracle at seeds 1, 2, 3, 5, and 8.
...
- `make fidelity-depth` — 4,638 total, 4,533 proven/delegated, 54 blocked,
...
The merged `waltz` handoff reported 4,626 total cases: 4,521
proven/delegated, 54 blocked, and 51 excluded. This slice adds twelve
proven/delegated rows, bringing the frontier to 4,638 total, 4,533
proven/delegated, 54 blocked, and 51 excluded. The next fresh social after
this slice is `wedgie` at `src/interpreter.c:811`.
