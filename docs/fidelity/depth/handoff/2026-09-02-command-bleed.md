...
`2026-09-02-command-blame.md`. The special-procedure inventory remains
...
`bleed`, at `src/interpreter.c:358`; bellow and bird remain claimed by their
open feature handoffs after their one-time CI retries.

The pre-slice frontier was 3,446 total cases, with 3,345 proven/delegated, 48
blocked, and 53 excluded. The bleed manifest contributes 11 fully
proven/delegated cases. The resulting frontier is:

- 3,457 total cases
- 3,356 proven/delegated
- 48 blocked
- 53 excluded

Actionable completion is 3,356/3,404 = 98.6%.
...
`bleed` record in `lib/misc/socials:1292-1300` has hide=0, a
...
`--show-oracle`. Every completed run reported `result: no normalized divergence`,
and the shown C blocks confirmed the intended branches. No Go behavior change
was needed: the generic `do_action` implementation was already byte-faithful.

Durable evidence is:

- `cmd/dp-oracle-diff/scenarios/bleed-depth.txt`;
- `cmd/dp-oracle-diff/scenarios/bleed-sleeping-depth.txt`;
- `docs/fidelity/depth/bleed.tsv`; and
- `pkg/session/bleed_depth_test.go`.
...
The required local gates passed on `glm/depth-bleed`:
...
Feature commit: `3d8bc8d2b`.

Feature PR: #1199 (`glm/depth-bleed`). Hosted lint, security, and test
...
`main` merge commit is `eb0eec12494ca814e290e734ecc0dc1eefc15372`.
...
The source-order sweep finds `bless` as the next genuinely unmanifested
interpreter family:
...
`bless` before advancing in interpreter-table order.
