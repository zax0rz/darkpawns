...
Added `cmd/dp-oracle-diff/scenarios/thpbt-depth.txt` with the standard actor
and peer fixture; `pkg/session/thpbt_depth_test.go` to pin the C command gate,
the social level/hide metadata, and all three parsed message slots; and
`docs/fidelity/depth/thpbt.tsv` with eight durable unit, delegated, and oracle
...
The final `thpbt-depth` matrix used the C oracle at seeds 1, 2, 3, 5, and 8.
...
- `make fidelity-depth` — 4,510 total, 4,405 proven/delegated, 54 blocked,
...
The handoff and evidence are in commits `a8a05fc68` and `8df5a8a93` at the
...
frontier to 4,510 total, 4,405 proven/delegated, 54 blocked, and 51 excluded.
The next fresh social after this slice is `threaten` at
