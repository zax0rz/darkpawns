...
`boink`, at `src/interpreter.c:363`.

The pre-slice frontier was 3,517 total cases, with 3,416 proven/delegated, 48
blocked, and 53 excluded. The boink manifest contributes 11 fully
proven/delegated cases. The resulting frontier is:

- 3,528 total cases
- 3,427 proven/delegated
- 48 blocked
- 53 excluded

Actionable completion is 3,427/3,475 = 98.6%.
...
`boink` record in `lib/misc/socials:56-65` has hide=0, a zero victim-position
...
- `cmd/dp-oracle-diff/scenarios/boink-depth.txt`;
- `cmd/dp-oracle-diff/scenarios/boink-sleeping-depth.txt`;
- `docs/fidelity/depth/boink.tsv`; and
- `pkg/session/boink_depth_test.go`.
...
The required local gates passed on `glm/depth-boink`:
...
Feature commit: `d3809d801`.

Feature PR: #1209 (`glm/depth-boink`). Hosted lint, security, and test
...
`main` merge commit is `f90bfa026bb290308f916f7addc17ca84f50336d`.
...
The source-order sweep finds `boggle` as the next genuinely unmanifested
interpreter family:
...
`boggle` before advancing in interpreter-table order.
