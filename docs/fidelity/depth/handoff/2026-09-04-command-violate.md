# Depth-fidelity handoff — `violate`

Date: 2026-09-04

Feature branch: `glm/depth-violate`

## Queue position and scope

This slice starts from merged main at `9f5e242f1` after the `twiddle` depth
...
in `src/interpreter.c` is `violate` at line 801. No `violate` manifest,
...
through `act`. The authored record at `lib/misc/socials:1138-1146` is:
...
Added `cmd/dp-oracle-diff/scenarios/violate-depth.txt` with the standard actor,
observer, target, and generic-mob fixture;
`pkg/session/violate_depth_test.go` to pin the C command gate, social metadata,
and all eight parsed message slots; and `docs/fidelity/depth/violate.tsv` with
...
The final `violate-depth` matrix used the C oracle at seeds 1, 2, 3, 5, and 8.
...
- `make fidelity-depth` — 4,614 total, 4,509 proven/delegated, 54 blocked,
...
The handoff and evidence are in commits `c64e3cee9` and `a43677328` at the
time of this note. No file under `src/` or `darkpawns-c-oracle/` was edited.

## Starting frontier

The merged `twiddle` handoff reported 4,602 total cases: 4,497
proven/delegated, 54 blocked, and 51 excluded. This slice adds twelve
proven/delegated rows, bringing the frontier to 4,614 total, 4,509
proven/delegated, 54 blocked, and 51 excluded. The next fresh social after
this slice is `waltz` at `src/interpreter.c:807`.
