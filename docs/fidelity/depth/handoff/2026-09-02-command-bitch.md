...
`2026-09-02-command-bird.md`. The special-procedure inventory remains
...
`bitch`, at `src/interpreter.c:355`; bellow and bird remain claimed by their
open feature handoffs after their one-time CI retries.

The pre-slice frontier was 3,424 total cases, with 3,323 proven/delegated, 48
blocked, and 53 excluded. The bitch manifest contributes 11 fully
proven/delegated cases. The resulting frontier is:

- 3,435 total cases
- 3,334 proven/delegated
- 48 blocked
- 53 excluded

Actionable completion is 3,334/3,382 = 98.6%.
...
`bitch` record in `lib/misc/socials:1178-1187` has hide=0, a
...
- `cmd/dp-oracle-diff/scenarios/bitch-depth.txt`;
- `cmd/dp-oracle-diff/scenarios/bitch-sleeping-depth.txt`;
- `docs/fidelity/depth/bitch.tsv`; and
- `pkg/session/bitch_depth_test.go`.
...
The required local gates passed on `glm/depth-bitch`:
...
Feature commit: `e90be5dbb`.

Feature PR: #1195 (`glm/depth-bitch`). Hosted lint, security, and test
...
`main` merge commit is `db4bd175785da90177f8f8759b194d20f8671c98`.
...
The source-order sweep finds `blame` as the next genuinely unmanifested
interpreter family:
...
`blame` before advancing in interpreter-table order.
