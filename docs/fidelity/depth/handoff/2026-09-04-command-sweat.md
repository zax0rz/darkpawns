...
Added `cmd/dp-oracle-diff/scenarios/sweat-depth.txt` with the standard actor
and peer fixture; `pkg/session/sweat_depth_test.go` to pin the C command gate,
the social level/hide metadata, and all three parsed message slots; and
`docs/fidelity/depth/sweat.tsv` with eight durable unit, delegated, and oracle
...
The final `sweat-depth` matrix used the C oracle at seeds 1, 2, 3, 5, and 8.
...
- `make fidelity-depth` — 4,442 total, 4,337 proven/delegated, 54 blocked,
...
The handoff and evidence are in commits `df6abb9f6` and `8bbed2fa7` at the
...
frontier to 4,442 total, 4,337 proven/delegated, 54 blocked, and 51 excluded.
The next fresh social after this slice is `tackle` at
