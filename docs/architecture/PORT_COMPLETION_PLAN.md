# Dark Pawns — Port Completion Plan

**Created:** 2026-04-26
**Status:** ✅ PORT COMPLETE (2026-04-27)

All tiers finished. Opus 5-pass review complete. 2 of 14 CRITICAL fixes applied.
See docs/reviews/README.md for current fix progress.
...
## TIER 1 ✅ DONE  # — Port Mechanics (C has it, Go must have it)

### Batch 1: Core Game Loop Gaps (~800 lines)
- [x] **KK_JIN/KK_ZHEN** — `affected_by_spell` infrastructure + wire into HitGain/MoveGain
...
- [x] **AFF_FLESH_ALTER** in level-up (GainExp + GainExpRegardless)
  - C ref: `src/class.c` — prevents level gain if AFF_FLESH_ALTER active
  - TODOs: `pkg/game/limits.go` lines 834, 869

- [x] **Full idle handling** — void pull, disconnect, timer tracking
...
- [x] **Weather graph penalty** — `pkg/game/graph.go:179`
- [x] **Zone mob AI dispatcher** — `pkg/game/zone_dispatcher.go:126`
- [x] **World target resolution** — `pkg/game/world.go` lines 1019, 1341
- [x] **World death handling** — `pkg/game/world.go:1007`
- [x] **Death reconnection/resurrection** — `pkg/game/death.go` lines 14, 311
- [x] **Other settings bug/typo/idea write** — `pkg/game/other_settings.go:104`

---

## TIER 3 ✅ DONE  # — Reviews (post-completion)

- [x] **Sonnet QA review** — fidelity to C source, correctness, edge cases
...
- [x] **Opus security review** — input validation, overflow, race conditions, auth
...
## Post-Review: Production Readiness  ✅ DONE  #

- [x] Address all review findings
- [x] Remove .bak files
- [x] Final `go vet ./...` + `go test ./...`
- [x] Update NEXT_SESSION.md with "PORT COMPLETE" status
- [x] GBrain page update: darkpawns/port-status
