<!--
Provenance: written by DeepSeek on 2026-09-24 from a read-only brief, reading
only the C oracle tree (/home/zach/darkpawns-c-oracle). Committed unedited below
this header. Per docs/research/README.md this is a field note: treat its claims
as leads until they are checked against a primary artefact.

Checked by Claude against the source on 2026-09-24 (PF-024):
- create_event is commented out of cmdlib (src/scripts.c:1616, "needs 3.0 event system").
- cuchi.lua sets ch.level = LVL_IMPL for a character named Orodreth (l.17-21).
  Not exploitable in the Go port: its table write-back covers only hp and gold,
  and save_char is an empty stub.
- start_room.lua carries the "dark pawns above which you must struggle" line.
- mob/archive/.svn/entries: last-author jravn, committed 2008-04-27.
- The Go engine registers create_event, mxp, get_group_lvl and skill_group
  (pkg/scripting/engine.go:646-675), none of which C's cmdlib has.
- cuchi.lua is attached to mob 18306 in the Go world (DP-1333).
Everything else here is DeepSeek's reading and is unverified.
-->
