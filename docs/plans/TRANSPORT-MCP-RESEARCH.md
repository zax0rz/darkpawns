...
>
> **Recovery note (2026-08-15):** the original `research/transport-mcp-2026-07-17/`
> directory was lost. This full text was reconstructed from the Claude session log
> `~/.claude/projects/-Users-zach--openclaw-workspace-darkpawns-repo/9cd09694-...jsonl`
> (mac). The companion `brief.md` and `findings/F1–F8.md` were written to disk by a
> deep-research subagent and never echoed back into the session, so they were not
> recoverable; their conclusions are folded into this report. Linear projects:
> DP-1140..DP-1152 (human transport, Option B `/wstelnet`).
