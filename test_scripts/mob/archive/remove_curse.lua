-- remove_curse.lua
-- Source: scripts_full_dump.txt lines 4461-4668
-- Original: NPC that removes ITEM_NODROP from items for (level*50) gold per item.
-- Greets players with cursed items, offers service via "remove <item>" command.
-- NOT a spell-based healer — operates on object extra flags directly via obj_extra().
-- Triggers: sound, greet, curse, oncmd()

function sound()
  -- Source: scripts_full_dump.txt lines 4462-4475
...
function greet()
  -- Source: scripts_full_dump.txt lines 4477-4498
  -- Count cursed (ITEM_NODROP) items in inventory + equipment
  local count = 0
...
function curse()
  -- Source: scripts_full_dump.txt lines 4500-4505
...
function oncmd()
  -- Source: scripts_full_dump.txt lines 4507-4565
  local command = ""
  local subcmd = ""
