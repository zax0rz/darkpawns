-- Source: scripts_full_dump.txt ./mob/archive/prisoner.lua
-- prisoner.lua — onpulse_pc causes slow poison damage; sound plays distress emotes;
-- ongive: if player gives antidote (vnum 18281) the prisoner is cured and rewards the
-- player with key (18280) and diary (12900); if given key (12901) the prisoner escapes
-- and gives 20000 gold. Attached to mob 18245.
  ...
  for i = 1, getn(me.objs) do
    if (me.objs[i].vnum == 18281) then
      if (aff_flagged(me, AFF_POISON)) then
        ...
        oload(ch, 12900, "char")
        spell(me, NIL, SPELL_REMOVE_POISON, FALSE)
        break
      else
