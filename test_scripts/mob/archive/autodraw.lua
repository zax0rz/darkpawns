-- autodraw.lua
-- Source: scripts_full_dump.txt ./mob/archive/autodraw.lua
-- Automated draw/lottery mob. Manages a group draw for an item.
-- Uses create_event for delayed tell delivery.
-- TODO: requires create_event implementation, use a no-op stub

-- Global state (persists across trigger calls within the same Lua state)
-- autodraw_char: the player who initiated the draw
-- drawers: table of participants
  ...
  if (command == "say") then
    if ((strlower(subcmd) == "begin") or (strlower(subcmd) == "begin.")) then
      if (ch.name ~= autodraw_char.name) then
        ...
        -- TODO: requires create_event implementation
