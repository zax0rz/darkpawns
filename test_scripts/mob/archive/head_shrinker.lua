-- Source: scripts_full_dump.txt ./mob/archive/head_shrinker.lua
-- head_shrinker.lua — oncmd that converts player heads (items with .owner set) into
-- a wearable necklace (vnum 7925) for gold. Attached to mob 7920.
-- TODO: requires extra(obj, str) implementation — sets extra description on object
-- TODO: requires strlen(s) implementation — string length helper
...
-- time. Attached to mob 7920.
  ...
  elseif (command == "buy") then
    ...
    extra(necklace, buf)			-- TODO: requires extra() implementation
  ...
  if (ch.objs) then
    for i = 1, getn(ch.objs) do
      if (ch.objs[i].owner) then
        ...
        length = strlen(ch.objs[i].owner)	-- TODO: requires strlen() implementation
