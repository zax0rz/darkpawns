...
**Updates from Apr 23 Deep Research — 6 new decisions applied:**
1. Decay model: time-based → power-law decay influenced by intermediate experience count (not wall clock). Neutral: 30-60d effective half-life. |valence|>=2: 90d.
2. Structured semantic representation: add Memori-like semantic triples as optional high-fidelity storage tier alongside free-text mem0. Target: 5x token reduction.
3. Procedural code-as-memory: combat tactics stored as executable rules, not NL memories. Prevents catastrophic forgetting of learned behavior.
4. Goal preference matrix: ATM-inspired weighted selection by context (combat/social/explore/loot). Maximum-weight for exploit, probabilistic for explore.
5. Active forgetting: prune memories with zero retrievals after 3 consecutive weekly checks. Archive to cold storage, don't delete.
6. Version-reflective cold storage: Light→REM→Deep pipeline preserves original episodes for authoritative lookup when memory contradicts observation.

---

### 🔲 Phase 5e — Tiered Inference & Mid-Context Defense
**Goal:** Reduce LLM cost by 40-70% and prevent context window saturation as BRENDA accumulates sessions.

**Tiered inference dispatch:**
- Router classifies decision type (combat/navigation/social/loot/meta) before dispatching
- Reflexive (navigation, loot, basic combat reactions) → cheap/fast model (Haiku, minimax, local Ollama)
- Tactical (multi-step planning, group tactics) → mid model (Sonnet, K2.6)
- Strategic (social reasoning, meta, narrative) → full model (Opus, GLM-5.1)
- Projected 40-70% cost reduction based on literature

**Mid-context defense:**
- Memori-style structured summaries for bootstrap injection
- Target: injected context stays under 2K tokens even after 20+ sessions
- Current ~2K after 5 sessions needs to stay flat or grow sublinearly
- Hierarchical abstractors: compress session logs before injection

**Client-side router for now** — `dp_brenda.py` decision classifier. Server-side dispatch deferred to Phase 6.

**Research value:** Tiered inference with in-game context classification is not well-documented in game AI literature. The cost reduction data would be publishable.
