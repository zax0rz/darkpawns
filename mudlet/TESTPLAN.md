...
| 9 | Close the gate again. Quit Tester and log Mudlet in as Warden. Type `auto exits` if the exit line is missing (C has no `autoexit` command), then `look` at the gate. | Warden's `[ Exits: ]` shows `(west)` in parentheses, and `gmcp.Room.Info.exits` includes `w`. Immortals see closed exits. |
