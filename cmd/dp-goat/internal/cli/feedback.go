...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".dp-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DP_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DP_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dp-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.dp-goat-pp-cli/feedback.jsonl.
When ` + "`DP_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DP_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dp-goat-pp-cli feedback list
  dp-goat-pp-cli feedback list --limit 5
  dp-goat-pp-cli feedback list --json`,
