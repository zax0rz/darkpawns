...
> Historical handoff from 2026-06-17, archived 2026-09-09. The statuses below
> are original notes, not a current task queue. In particular, the database
> claim is obsolete: `cmd/server/main.go` now reads `DATABASE_URL` and rejects
> database initialization failure unless `DP_ALLOW_NO_DB=1`. See
> [the current running guide](../DEPLOYMENT.md). Other items need verification
> against code and Git history before treating them as open or resolved.
